...
from spinaiocr.data.augment import clahe
from spinaiocr.inference.db_postprocess import DBPostProcessor
from spinaiocr.inference.decoders import BeamConfig, CharBigramLM, decode_batch
from spinaiocr.inference.edge_refine import EdgeRefiner, SnapConfig
from spinaiocr.log import get_logger, log_span
from spinaiocr.models.detection import DBNet
from spinaiocr.models.recognition import build_recognition
from spinaiocr.vocab.base import Vocab, load_vocab

log = get_logger("spinaiocr.inference")
...
class OCRPipeline:
    def __init__(
        ...
        from spinaiocr.models.weights import ensure_models
    ...
    def _load_base_corpus(cls) -> list[str]:
        ...
        from spinaiocr.data.synth_simple import default_korean_corpus
    ...
    def _vlm_readtext_multi(self, img: np.ndarray) -> list[tuple[np.ndarray, str, float]]:
        ...
        if not multi:
            return []
        from spinaiocr.inference.vl_client import vl_ocr, vl_healthy
        if not vl_healthy():
    ...
    def __call__(
            ...
            with log_span("pipeline.single_line"):
                ...
                if not texts[0] and _single_line_auto:
                    ...
                    if self.config.recognition.use_jamo_correction and _sl_text:
                        from spinaiocr.postprocess.jamo_correct import correct_jamo
        ...
        with log_span("pipeline.recognize", n_polys=len(polygons)):
            ...
            if tta_enabled and low_idx:
                from spinaiocr.inference.tta_ensemble import tta_predict, TTAConfig
    ...
    def _get_llm_corrector(self):
        if self._llm_corrector is None:
            from spinaiocr.postprocess.llm import CorrectorConfig, LLMCorrector
    ...
    def _apply_angle_correction(self, crops: list) -> list:
        ...
        if self._angle_classifier is None:
            ...
            if ckpt_path.exists():
                from spinaiocr.models.angle_cls import AngleClassifier
        ...
        from spinaiocr.models.angle_cls import predict_angle, rotate_to_upright
        corrected = []
        n_rotated = 0
