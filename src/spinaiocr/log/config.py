...
from spinaiocr.log.formatters import ConsoleFormatter, JsonLinesFormatter
...
class LogConfig:
    ...
    def from_env(cls) -> "LogConfig":
        ...
        Per-module overrides are supplied via ``SPINAI_LOG_LEVELS`` as JSON:
            SPINAI_LOG_LEVELS='{"spinaiocr.teachers": "DEBUG"}'
        """
        cfg = cls(
...
def _install_excepthook(cfg: LogConfig) -> None:
    ...
    def _hook(exc_type, exc, tb):
        logging.getLogger("spinaiocr.unhandled").critical(
            "unhandled exception", exc_info=(exc_type, exc, tb)
        )
        from spinaiocr.log.context import write_crash_dump
