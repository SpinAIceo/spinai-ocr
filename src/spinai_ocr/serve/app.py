...
Run:
    uvicorn spinai_ocr.serve.app:app --host 0.0.0.0 --port 8080
"""
...
from spinai_ocr import __version__
from spinai_ocr.config import DecodeMode, LangCode, ModelTier, PipelineConfig
from spinai_ocr.inference.pipeline import OCRPipeline
from spinai_ocr.log import get_logger, setup_logging
from spinai_ocr.serve.ratelimit import (
    check_monthly_quota, commit_monthly_call, get_usage,
)
from spinai_ocr.serve.hard_negatives import maybe_queue_hard_negative
from spinai_ocr.serve.request_log import (
    log_ocr_request, read_recent, aggregate, image_fingerprint,
)

setup_logging()
log = get_logger("spinai_ocr.serve.app")
...
class _SSELogHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        try:
            from spinai_ocr.log.formatters import JsonLinesFormatter
...
logging.getLogger("spinai_ocr").addHandler(_sse_handler)
